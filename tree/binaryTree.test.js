...
beforeEach(function () {
  ...
  // build small tree;
  let smallLeft = new Node(5);
  let smallRight = new Node(5);
  let smallRoot = new Node(6, smallLeft, smallRight);
  ...
  let node3 = new Node(3, node4, node6);
  let node2 = new Node(5, node3, node5);
  let node1 = new Node(5);
  let root = new Node(6, node1, node2);
...
describe("maxSum", function () {
  ...
  it("handles negative values", function () {
    ...
    let nodeNeg4 = new Node(-4);
    let node2 = new Node(2, nodeNeg4);
    let nodeNeg3 = new Node(-3, node8, node100);
    let root = new Node(10, node2, nodeNeg3);
...
describe("serialize and deserialize", function () {
  ...
  it("deserializes strings into BinaryTree objects", function () {
    // Failure message:
    // The 'deserialize' function needs to output a Node
...
describe("lowestCommonAncestor", function () {
  it("returns the lowest common ancestor", function () {
    ...
    const leftLeft = new Node(6);
    left.left = leftLeft;

    const leftRight = new Node(2);
    left.right = leftRight;

    const leftRightLeft = new Node(7);
    leftRight.left = leftRightLeft;

    const leftRightRight = new Node(4);
    leftRight.right = leftRightRight;

    /* build right subtree */

    const right = new Node(1);
    root.right = right;

    const right_left = new Node(0);
    right.left = right_left;

    const right_right = new Node(8);
